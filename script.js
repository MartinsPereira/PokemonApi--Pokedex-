...
const inputBtn = document.querySelector('.pokemons > header > nav > input[type="submit"]')
const btnMostrar = document.querySelector('.btnMostarMais button')
const backgroundPokemon = document.querySelector('.imgWindowPokemon')
let a = 0;
let tipos = [];

async function activePokemon(){
        const request = await fetch(`https://pokeapi.co/api/v2/pokemon/?offset=${a}&limit=20`)
        const response = await request.json();
        response.results.map((item, index) => {
                activePokemonList(item);
...
async function activePokemonList(item){
    const request1 = await fetch('https://pokeapi.co/api/v2/pokemon/' + item.url.substr(-4).replace('/','').replace('/','').replace('n',''))
    const response1 = await request1.json(); 
    let pokemonIndividual = document.querySelector('.models .pokemonIndividual').cloneNode(true);

    pokemonIndividual.setAttribute('data-key', item.url.substr(-4).replace('/','').replace('/','').replace('n',''))
    pokemonIndividual.querySelector('.imgPokemon').src = Object.entries(response1.sprites)[4][1];
...
async function activePokemonInput(){
    let valueInput = inputProcurar.value.toLowerCase();
    console.log(valueInput)
    const request1 = await fetch('https://pokeapi.co/api/v2/pokemon/' + valueInput)
    const response1 = await request1.json(); 
    const pokemonList = document.querySelectorAll('.listaPokemon .pokemonIndividual')
    let pokemonIndividual = document.querySelector('.models .pokemonIndividual').cloneNode(true);

    pokemonIndividual.setAttribute('data-key', response1.id)
    pokemonIndividual.querySelector('.imgPokemon').src = Object.entries(response1.sprites)[4][1];
    pokemonIndividual.querySelector('.nomePokemon').innerHTML = response1.name.replace(/\w/, c => c.toUpperCase());
    ...
    pokemonList.forEach(item => {
        document.querySelector('.listaPokemon').replaceChild(pokemonIndividual, item)
    })
    a = 0;
...
function pokemonClickModel(pokemonQuery){
    pokemonQuery.addEventListener('click', (event)=> {
        let key = event.target.closest('.pokemonIndividual').getAttribute('data-key')
        fetch('https://pokeapi.co/api/v2/pokemon/' + key).then((requestPoke) => requestPoke.json())
        .then(responsePoke => {
            document.querySelector('.imgWindowPokemon img').src = Object.entries(responsePoke.sprites)[4][1]
            document.querySelector('.nomeWindowPokemon').innerHTML = responsePoke.name.replace(/\w/, c => c.toUpperCase());
            for(let i=0; i < responsePoke.types.length ;i++){
                tipos.push(responsePoke.types[i].type.name);
            ...
            if(responsePoke.height.toString().length < 2){
                document.querySelector('.pesoAlturaPokemon span:nth-child(1)').innerHTML = 'Altura: 0,' + responsePoke.height + ' m'
            }
            else{
                document.querySelector('.pesoAlturaPokemon span:nth-child(1)').innerHTML = 'Altura: ' + responsePoke.height.toString().substring(0,responsePoke.height.toString().length -1 ) + ',' + responsePoke.height % 10 + ' m'
            }

            if(responsePoke.weight.toString().length < 2){
                document.querySelector('.pesoAlturaPokemon span:nth-child(2)').innerHTML = 'Peso: 0,' + responsePoke.weight + ' kg'
            }
            else if(responsePoke.weight.toString().length < 3){
                document.querySelector('.pesoAlturaPokemon span:nth-child(2)').innerHTML = 'Peso: ' + responsePoke.weight.toString().replace(responsePoke.weight.toString().slice(-1),',')+ responsePoke.weight.toString().substr(-1) + ' kg'
            }else{
                document.querySelector('.pesoAlturaPokemon span:nth-child(2)').innerHTML = 'Peso: ' + responsePoke.weight.toString().substring(0, responsePoke.weight.toString().length - 1) + ',' + responsePoke.weight % 10  + ' kg'
            }

            document.querySelector('.hpPokemonNum').innerHTML = responsePoke.stats[0].base_stat 
            document.querySelector('.atkPokemonNum').innerHTML = responsePoke.stats[1].base_stat
            document.querySelector('.defPokemonNum').innerHTML = responsePoke.stats[2].base_stat
            document.querySelector('.spAtkPokemonNum').innerHTML = responsePoke.stats[3].base_stat
            document.querySelector('.spDefPokemonNum').innerHTML = responsePoke.stats[4].base_stat
            document.querySelector('.speedPokemonNum').innerHTML = responsePoke.stats[5].base_stat

            if(window.innerWidth <= 900){
                setTimeout(() => {
                    document.querySelector('.progressBar1').style.width = responsePoke.stats[0].base_stat + 'px'
                    document.querySelector('.progressBar2').style.width = responsePoke.stats[1].base_stat + 'px'
                    document.querySelector('.progressBar3').style.width = responsePoke.stats[2].base_stat + 'px'
                    document.querySelector('.progressBar4').style.width = responsePoke.stats[3].base_stat + 'px'
                    document.querySelector('.progressBar5').style.width = responsePoke.stats[4].base_stat + 'px'
                    document.querySelector('.progressBar6').style.width = responsePoke.stats[5].base_stat + 'px'
                },600)
    
    
                document.querySelector('.pokemonWindow').style.opacity = 0
                document.querySelector('.pokemonWindow').style.display = 'flex'
                setTimeout(()=> {
                    document.querySelector('.pokemonWindow').style.opacity = 1
                },200)
            }else{
                ...
                setTimeout(()=> {
                    document.querySelector('.pokemonWindow').style.opacity = 1
                    setTimeout(() => {
                        document.querySelector('.btnCloseWindow').classList.add('ativoBtnPokemon')
                        document.querySelector('.dadosPokemon').classList.add('ativoDadosPokemon')
                        document.querySelector('.imgWindowPokemon').classList.add('ativoImgWindow')
                    },200)
                },200)
            }
...
function closeWindow(){
    if(window.innerWidth <= 900){
        document.querySelector('.pokemonWindow').style.opacity = 0
        setTimeout(()=> {
            document.querySelectorAll('.atributosWindowPokemon span').forEach(item => {
                document.querySelector('.atributosWindowPokemon').removeChild(item)
            })
            document.querySelector('.pokemonWindow').style.display = 'none'
        },200)
    }else{
        document.querySelector('.btnCloseWindow').classList.remove('ativoBtnPokemon')
        document.querySelector('.dadosPokemon').classList.remove('ativoDadosPokemon')
        document.querySelector('.imgWindowPokemon').classList.remove('ativoImgWindow')
        setTimeout(() => {
            document.querySelector('.pokemonWindow').style.opacity = 0
        },1000)
        setTimeout(()=> {
